						...
						<c:forEach var="dto" items="${list}">
							<div class="col-md-4">
								<div class="card" style="width: 18rem;">
									<a onclick="window.location='${path}/product_Detail.do?pdNo=${dto.pdNo}&pageNum=${paging.pageNum}'">
							...
							<tr>
								<td colspan="12" align="center">
									<!-- 페이징 처리 --> <!-- 이전 버튼 활성화 여부 --> <c:if
										...
										<a href="${path}/product_flower.do?pageNum=${paging.prev}">[이전]</a>
									</c:if> <!-- 페이지 번호 처리 --> <c:forEach var="num"
										begin="${paging.startPage}" end="${paging.endPage}">
										<a href="${path}/product_flower.do?pageNum=${num}">${num}</a>


									</c:forEach> <!-- 다음 버튼 활성화 여부 --> <c:if
										test="${paging.endPage < paging.pageCount}">
										<a href="${path}/product_flower.do?pageNum=${paging.next}">[다음]</a>
									</c:if>
								</td>
							</tr>
							</table>
