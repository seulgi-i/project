...
public class OrderServiceImpl implements OrderService {
	...
	public void orderPageAction(HttpServletResponse res, HttpServletRequest req) 
		...
		for (int i = 0; i < pdNo_arr.length; i++) {
			...
			dto = dao.orderPage(Integer.parseInt(pdNo_arr[i]), strId);
			list.add(dto);
	...
	// 주문요청 ( 주문목록 추가 처리 )
	@Override
	public void orderAddAction(HttpServletResponse res, HttpServletRequest req) throws ServletException, IOException {
		...
		for (int i = 0; i < pdNo_arr.length; i++) {
			...
			if (list != null) {
				...
				//장바구니에서 주문상품 삭제
				deleteCnt = cdao.cartDelete(pdNo, strId); 
			}
			// 6단계. jsp로 처리결과 전달
