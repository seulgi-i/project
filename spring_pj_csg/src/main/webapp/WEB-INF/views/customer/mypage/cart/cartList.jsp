	...
	$(document).ready(function() {
		...
		$(function() {
			...
			$("#selectDel").click(function(){
			  $(".cart_info_td").each(function(index, element) {
			 	...
			 	if($(element).find(".checkBox").is(":checked") === true) { 
					document.editForm.action = "${path}/CartDeleteAction.cr?${_csrf.parameterName}=${_csrf.token}";
		   ...
		   $("#btnadel").click(function() {
			  document.editForm.action = "${path}/CartAllDeleteAction.cr?${_csrf.parameterName}=${_csrf.token}";
			  document.editForm.submit();
			});
		   ...
		   $("#btnbuy").click(function(){
			 $(".checkBox").prop("checked", true);  
			 	$(".cart_info_td").each(function(index, element) {
					//현재 배열의 순서 중 요소를 찾는다. true된 현재 값을 모두 전송
				  if($(element).find(".checkBox").is(":checked") === true) { 
					document.editForm.action = "${path}/OrderPageAction.or?${_csrf.parameterName}=${_csrf.token}";
					document.editForm.submit();
					}
				 });
			   });
							...
							<c:forEach var="dto" items="${list}" varStatus="status">
								<tr class="cart_info_td">
									...
									<td><input type="number" name="cartCnt" class="cartCnt"
										value="${dto.cartCnt}" min="1" max="99"
										style="width: 43px; text-align: center"> <input
										type="button" class="updateButton" value="변경"
										data-num="${dto.cartNo}"
										style="border: 1px solid; background: transparent;"> <input
										type="hidden" class="cart_sum" 
										value="${dto.price * dto.cartCnt}"></td>

									<td><input type="hidden" id="price" value="${dto.price}">
										<fmt:formatNumber pattern="###,###,###" value="${dto.price * dto.cartCnt}" />
										원</td>
								...
								<td colspan='8' align='center'><br> · 15만원 이상 구매시 무료배송
									...
									<div class="orderInfo-price">
										...
										<button type="button" id="btnList"
											onclick="window.location='${path}/product_flower.do'">상품목록</button>
