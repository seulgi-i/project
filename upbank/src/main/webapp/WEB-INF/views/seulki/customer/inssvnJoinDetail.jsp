...
function input_Text12(){
	    document.getElementById("i_end_date").value = '12';
	}	
function input_Text24(){
	    document.getElementById("i_end_date").value = '24';
	}
function input_Text36(){
	    document.getElementById("i_end_date").value = '36';
	}
function input_Text1000(){
	document.getElementById("i_money").value = '10000000';
}
function input_Text500(){
	document.getElementById("i_money").value = '5000000';
}
function input_Text300(){
	document.getElementById("i_money").value = '3000000';
}
function input_Text100(){
	document.getElementById("i_money").value = '1000000';
}
...
$(function() {
	...
	$("#sumit_b").click(function() {
		document.inssvnJoinForm.action="${path}/inssvnJoin.do?${_csrf.parameterName}=${_csrf.token}";
		document.inssvnJoinForm.submit();
	});
});
		...
		<div class="container main-content clearfix">
			...
			<div class="eleven columns bottom-3">
				...
				<div class="post single style-1">
					...
					<h3 class="title bottom-1">${dto2.i_name}</h3>

					<h5>정보입력</h5>
					<hr style="border: solid 2px #378AC4">
					<br>
					<!-- Title Post -->
					<form name="inssvnJoinForm" action="${path}/inssvnJoin.do"
						method="post">
						<input type="hidden" name="${_csrf.parameterName}"
							value="${_csrf.token}"> <input type="hidden"
							name="i_name" value="${dto2.i_name}"> <input
							type="hidden" name="i_type" value="${dto2.i_type}"> <input
						...
						<table class="style color" width="600px">
							<tr>
								<th>가입기간</th>
								<td><input type="text"
									style="margin-top: 15px; text-align: center;" size="20"
									name="i_end_date" id="i_end_date" placeholder="1~36개월, 월단위">개월
									<c:if test="${dto2.i_max_date==36}">
									...
									</c:if> <c:if test="${dto2.i_max_date==24}">
										<input type="button" id="btn" name="dupChk"
											onclick="input_Text12()" value="12개월">
										<input type="button" id="btn" name="dupChk"
											onclick="input_Text24()" value="24개월">
									</c:if> <c:if test="${dto2.i_max_date==12}">
										<input type="button" id="btn" name="dupChk"
											onclick="input_Text12()" value="12개월">
									</c:if></td>
							</tr>
							...
							<tr>
								...
								<br>
								<td><input type="text"
									style="margin-top: 15px; text-align: center;" size="25"
									name="i_money" id="i_money" placeholder="최소100만원 이상, 원단위">원
									<input type="button" id="btn" name="dupChk"
						...
						<tr>
							...
							<tr>
								...
								<td><input type="password" id="2ACCOUNT_PW"
									name="2ACCOUNT_PW" maxlength="4" placeholder="숫자 4자리를 입력하세요"
									required></td>
							</tr>
							<tr>
								<th>신규 비밀번호</th>
								<td><input type="password" id="ACCOUNT_PW"
									name="ACCOUNT_PW" maxlength="4" placeholder="숫자 4자리를 입력하세요"
									required></td>
							</tr>
							<tr>
								<th>비밀번호 확인</th>
								<td><input type="password" id="ACCOUNT_PW_RE"
									name="ACCOUNT_PW_RE" maxlength="4" placeholder="숫자 4자리를 입력하세요"
									required></td>
						</table>
