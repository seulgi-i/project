...
/* 버튼명 중복가능하도록 ^이용해서 만들고, 마지막에 html 하나로통일하는법 고민해보기*/
 ...
 $(function(){
	$("#sleepBtn").click(function(){
		if(confirm("일괄 휴면처리 하시겠습니까?")){
			if(${list!=null}){
				location.href = '${path}/adminAccountChange_sleep.ad';
			}else{
				alert("휴면처리할 항목이 없습니다")
			}
		}
	})
 })
...
<body>
   ...
   <!-- Start Posts -->
     ...
     <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
      ...
      <h3 class="title bottom-1">휴면대상 계좌</h3><!-- Title Post -->
       <div class="container main-content clearfix" style="width:95%">
          ...
          <div id="content1" class="tabscontent">
	          ...
	          <tbody valign="middle">
	            ...
	            <tr style="border:none;">
	            	<td colspan=8 style="border:none;">
						...
						<c:if test="${paging.startPage > 10}">
							<a href="${path}/accountList_5year.ad?pageNum=${paging.prev}">[이전]</a>
						</c:if>
						
						<!-- 페이지 번호 처리 -->
						<!-- header.css div #right a{ color: black:} 추가 처리 -->
						<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
							<a href="${path}/accountList_5year.ad?pageNum=${num}">${num}</a>
						</c:forEach>
						
						<!-- 다음버튼 활성화 여부 -->
						<c:if test="${paging.endPage < paging.pageCount}">
							<a href="${path}/accountList_5year.ad?pageNum=${paging.next}">[다음]</a>
						</c:if>
					</td>
	            </tr>
	            ...
	            <tr style="border:none;">
	            	<td colspan=8 style="border:none;">
	            		<input type="button" id="sleepBtn" value="일괄휴면처리" class="button1 medium color bottom-2">
	            	</td>
	            </tr>
