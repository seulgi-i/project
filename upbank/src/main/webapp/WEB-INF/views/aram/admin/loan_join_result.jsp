...
<head>
  <!-- Basic Page Needs -->
  <meta charset="utf-8">
  <title> UpBank 대출 - 관리자 : 대출 신청 결과 조회</title>
				...
				<table class="style color">
		          <tbody>
		            ...
		            <c:forEach var="dto" items="${list}">
			            <tr>
			              <td>${dto.l_join_num}</td>
			              <td><a href="${path}/admin_loan_join_detail.lo?l_join_num=${dto.l_join_num}">${dto.loan_name}</a></td>
	     	...
	     	<div class="pagination" style="justify-content:center;">
				...
				<!-- 이전 버튼 활성화 여부 -->
	     		<c:if test="${paging.startPage > 10}">
					<a href="${path}/admin_loan_join.lo?pageNum=${paging.prev}" class="prev-button"><i class="icon-angle-left"></i></a>
				</c:if>
				<c:if test="${paging.startPage <= 10}">
	          ...
	          <!-- 페이지 번호 처리 -->
				<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
					<c:if test="${num == paging.pageNum}">
						<a href="${path}/admin_loan_join.lo?pageNum=${num}" class="current">${num}</a>
					</c:if>
					<c:if test="${num != paging.pageNum}">
						<a href="${path}/admin_loan_join.lo?pageNum=${num}">${num}</a>
					</c:if>
				</c:forEach>
