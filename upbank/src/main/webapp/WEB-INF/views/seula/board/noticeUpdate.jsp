   ...
   <!-- Start Posts -->
     <div class="sixteen columns bottom-3">
      ...
      <!-- form 안에 에디터를 사용하는 경우 (보통 이경우를 많이 사용하는듯)-->
		<form method="post" class="form-elements" id="insertForm" name="insertForm" action="${path}/noticeUpdateAction.do">
         <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
         <input type="hidden" name ="n_No" value="${dto.n_No}">
          <fieldset>
          <span>제목 : </span>
          <input type="text" name="n_title" class="title" id="title" value="${dto.n_title}">
          </fieldset>
		  <textarea id="summernote" name="n_content" class="content">${dto.n_content}</textarea>
          
		  <div  style=" display: flex; justify-content: center; margin-top:20px;">
		  
		  <input type="submit" value="수정" class="button1 small color" id="saveBtn">
          <input type="reset" value="목록" class="button1 small normal" onclick="window.location='${path}/notice.do'">
		  </div>
		</form>
